...
const MandrelV2ApiTest: React.FC = () => {
  ...
  const runAllTests = useCallback(async () => {
    ...
    try {
      // Test 1: Health Check
      await runTest('V2 Health Check', () => mandrelApi.getHealth());

      // Test 2: Ping
      await runTest('AIDIS Ping', () => mandrelApi.ping('Frontend integration test'));

      // Test 3: Get Status
      await runTest('Get Status', () => mandrelApi.getStatus());

      // Test 4: List Tools
      await runTest('List Tools', () => mandrelApi.listTools());

      // Test 5: Get Current Project
      await runTest('Get Current Project', () => mandrelApi.getCurrentProject());

      // Test 6: List Projects
      await runTest('List Projects', () => mandrelApi.listProjects(true));

      // Test 7: Get Session Status
      await runTest('Get Session Status', () => mandrelApi.getSessionStatus());

      // Test 8: Error Handling Test (intentionally bad request)
      await runTest('Error Handling Test', () =>
        mandrelApi.callTool('nonexistent_tool', {})
      );
...
export default MandrelV2ApiTest;