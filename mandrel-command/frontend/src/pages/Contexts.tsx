...
const Contexts: React.FC = () => {
  ...
  const handleContextShare = (context: Context, format: 'markdown' | 'text' | 'json') => {
    ...
    switch (format) {
      case 'markdown': {
        const markdownContent = `# 🎯 Mandrel Context: ${getTypeDisplayName(context.type)}
...
`;
      ...
      case 'text': {
        const textContent = `🎯 Mandrel Context: ${getTypeDisplayName(context.type)}
