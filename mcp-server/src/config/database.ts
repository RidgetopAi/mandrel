...
// Load order: .local files first (highest priority), then environment, then base
// .local files are gitignored and contain machine-specific overrides
const envPaths = [
  path.join(configRoot, 'environments', `.env.${nodeEnv}.local`),  // Local overrides (highest priority)
  path.join(configRoot, 'environments', `.env.${nodeEnv}`),        // Environment-specific
  path.resolve(__dirname, '../../.env')                             // Base fallback
];
